...
namespace Microsoft.Azure.Zumo.WindowsPhone7.CSharp.Test
{
    ...
    public class ZumoQuery : TestBase
    {
        ...
        public void FilterMethods()
        {
            ...
            query = Compile<Product, Product>(table =>
                from p in table
                where decimal.Floor(p.Price) == 10  //use decimal.Floor instead of Math.Floor in wp7
                select p);
            ...
            query = Compile<Product, Product>(table =>
                from p in table
                where decimal.Ceiling(p.Price) == 10 //use decimal.Ceiling instead of Math.Ceiling  conversion in wp7
                select p);
